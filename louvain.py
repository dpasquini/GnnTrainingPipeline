...
def main(cfg: DictConfig) -> None:
    ...
    for g in test:
        ...
        pesi = peso.tolist()
        part = gr.community_multilevel(weights=pesi)
    ...
    logger.info("Q non diretta media %.4f (quella che Louvain massimizza)",
                media("Q_non_diretta"))
    ...
    with open(cfg.paths.louvain, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(righe[0]))
        w.writeheader()
        w.writerows(righe)
    logger.info("dettaglio per grafo in %s", cfg.paths.louvain)
